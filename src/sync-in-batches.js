...
const CHUNK_SIZE = 20; // Define the size of each chunk
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (Object.keys(bynderAssets).length == 0 ||
          Object.keys(bynderAssets).length < 1000) { //TODO: random number to fail safe to figure out why bynder only returns 6 assets sometimes
...
async function loopThroughAllAssets(serverAssets, bynderAssets) {
  ...
  var assetsToUploadToBynderArray = [];
    ...
    if(foundInBynder){
      try {
       // console.log('Match found, comparing the assets ' +  serverAssetFileName);
        var isModified = await compareAsset(serverAsset, bynderAsset, assetsToUploadToBynderArray);
        if(isModified) {
           setServerAssetsTagging(serverAsset, assetsToUploadToBynderArray);
        }
      ...
      //upload array to Bynder in batches of 50
     await uploadAssetsInChunks(assetsToUploadToBynderArray);
...
async function setServerAssetsTagging(asset, assetsToUploadToBynderArray ) {
  return new Promise((resolve, reject) => {
    ...
    //   }
      ...
      if ('emphasis' in asset) {
       ...
       assetsToUploadToBynderArray.push(requestData); // Or whichever data you want to store
...
// Function to upload a batch of assets
async function uploadAssetsInChunks(assetsToUploadToBynderArray) {
  // Loop through the assets in chunks of CHUNK_SIZE
  for (let i = 0; i < assetsToUploadToBynderArray.length; i += CHUNK_SIZE) {
    const chunk = assetsToUploadToBynderArray.slice(i, i + CHUNK_SIZE); // Create a chunk of assets

    try {
      // Use Promise.all to upload all assets in the current chunk in parallel
      await Promise.all(
        chunk.map((asset) => {
          var requestData = { ...asset }; // Prepare requestData based on the asset
         
          return new Promise((resolve, reject) => {
            bynder.uploadFile(requestData)
              .then((data) => {
                if (data.success === true) {
                  console.log("Successfully uploaded asset: " + requestData.body.path + " to Bynder!");
                  resolve(); // Resolve the promise when upload is successful
                } else {
                  reject("(Promise Reject) Failed to upload asset: " + requestData.body.path + " to Bynder!");
                }
              })
              .catch((error) => {
                console.log("(Catch) Failed to upload asset: " + requestData.body.path + " to Bynder! " + error.data);
                reject(error);
              });
          });
        })
      ...
      console.log(`Successfully uploaded batch of ${chunk.length} assets.`);
    } catch (error) {
      console.error("Error uploading batch:", error);
      // Handle error as needed, e.g., logging or retry logic
    }
  }
...
//     try {
//       // Process each asset in the current chunk sequentially
...
//TODO: Delete in batches when given time to work on
...
async function checkBynderUnwantedFiles(serverAssets, bynderAssets){
  ...
  for (var bynderFilePath in bynderAssets) {
        ...
        if(!foundInServer){
          try {
            console.log("Bynder asset: " + bynderAssetName + " not found on the server...deleting from bynder" )
            await deleteBynderAsset(bynderAsset);
...
if( Object.keys(serverAssets).length == 0 || Object.keys(serverAssets).length < 200 ){
